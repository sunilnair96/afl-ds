...
OUTPUT_FILE = r"C:\Users\sunil\Projects\2026ws\afl-ds\data\raw\afl_fantasy_player_summary_ceiling.xlsx"
...
# -------------------------------------------------------------------
# MAIN LOGIC (CEILING VERSION – BEST 2 VS OPPONENT)
# -------------------------------------------------------------------

def consolidate_player_stats_ceiling():
        ...
        for _, row in df.iterrows():
            ...
            if player not in player_stats:
                player_stats[player] = {
                    ...
                    "ppm_sum": 0.0,
                    "row_count": 0,
                    # For ceiling: keep list of all scores vs each opponent
                    # so we can take the best 2 later.
                    "opp_scores": {},  # opp -> [scores...]
                }

            p = player_stats[player]

            # Aggregate base columns as before
            for col, key in [
                ("AVG", "avg_sum"),
                ("ADJ", "adj_sum"),
                ("100", "sum_100"),
            ...
            # Store all vs-opponent scores; we'll pick best 2 at the end
            for col in opp_cols:
                ...
                if opp_team not in p["opp_scores"]:
                    p["opp_scores"][opp_team] = []
                p["opp_scores"][opp_team].append(score)
    ...
    for player, p in player_stats.items():
        ...
        # Ceiling vs each opponent team:
        # average of best 2 scores (if only 1 game, just that score)
        for team in TEAMS:
            scores = p["opp_scores"].get(team, [])
            if scores:
                # Sort descending and take top 2
                top_scores = sorted(scores, reverse=True)[:2]
                row[team] = sum(top_scores) / len(top_scores)
            else:
                row[team] = None
    ...
    print(f"Ceiling summary (best 2 vs each opponent) written to {OUTPUT_FILE}")


if __name__ == "__main__":
    consolidate_player_stats_ceiling()
